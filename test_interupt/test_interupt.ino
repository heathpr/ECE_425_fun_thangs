...
int ultraSoundSignal = D1; // Ultrasound signal pinint val = 0;
 ...
 */
 if (ofStart != -1) {
   ...
   Robot.text("Interrput works!",5,1);
 }
}
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(ultraSoundSignal), ultrasound_response, FALLING);
...
void printValue() {
  ...
  ft = in / 12;
  in = in % 12;
  Robot.text(ultrasoundValue,5,9);
